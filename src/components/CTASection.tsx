...
export default function CTASection() {
  ...
  return (
    <section id="cta" className="relative py-14 sm:py-16 md:py-20 lg:py-28 overflow-x-hidden">
      ...
      <div className="max-w-6xl mx-auto px-6 sm:px-8 lg:px-12 relative z-10 text-center">
        ...
        <h2 className="font-syne font-extrabold leading-[0.92] tracking-[-0.02em] mb-6 sm:mb-8 drop-shadow-[0_8px_38px_rgba(0,0,0,0.45)]">
          <span className="block text-zs-text text-[clamp(2.1rem,8.6vw,6.5rem)]">{t.cta.line1}</span>
          <span className="block text-gradient-cyan text-[clamp(2.1rem,8.6vw,6.5rem)]">{t.cta.line2}</span>
          <span className="block text-zs-text text-[clamp(2.1rem,8.6vw,6.5rem)]">{t.cta.line3}</span>
        </h2>

        <p className="font-dm text-[clamp(0.98rem,2vw,1.2rem)] text-zs-muted/95 max-w-3xl mx-auto leading-relaxed sm:leading-[1.75] mb-8 sm:mb-10 text-left sm:text-center">
          {t.cta.sub}
        </p>
        ...
        <div className="grid grid-cols-1 xs:grid-cols-3 gap-3 max-w-3xl mx-auto">
          {([
            ...
            <div key={i} className="glass-card border border-zs-border rounded-xl p-4 sm:p-5 text-center">
              <div className={`font-mono text-[10px] sm:text-[11px] tracking-[0.1em] uppercase mb-2 font-bold ${
                item.color === "cyan" ? "text-zs-cyan" : item.color === "violet" ? "text-zs-violet-bright" : "text-zs-gold"
              }`}>{item.label}</div>
              <div className="font-syne font-semibold text-sm sm:text-base text-zs-text">{item.value}</div>
            </div>
          ))}
        </div>
      </div>
