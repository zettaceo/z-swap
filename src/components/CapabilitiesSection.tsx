    ...
    <section id="capabilities" className="relative py-12 sm:py-16 md:py-20 lg:py-24 overflow-x-hidden">
      ...
      <div className="max-w-7xl mx-auto px-6 sm:px-8 lg:px-12">
        ...
        <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-3 mb-8 sm:mb-10">
          ...
          <p className="font-dm text-sm sm:text-base text-zs-muted max-w-xl leading-relaxed">{t.capabilities.sub}</p>
        </div>
        ...
        <div className="flex flex-col lg:grid lg:grid-cols-[200px_1fr] xl:grid-cols-[220px_1fr] gap-4 sm:gap-6">
            ...
            {layers.map((l) => {
              ...
              return (
                <button
                  ...
                  className={`flex-shrink-0 text-left px-3.5 sm:px-5 py-3 rounded-xl border transition-all duration-200 ${
                    isActive
                      ? `glass-card ${lc.active} ${lc.text}`
                      : "border-transparent text-zs-muted hover:text-zs-text hover:bg-zs-faint/20"
                  }`}
                >
                  <div className={`font-mono text-[11px] tracking-[0.16em] uppercase mb-1 ${isActive ? lc.text : "text-zs-faint"}`}>
                    {l.tag}
                  </div>
                  <div className="font-syne font-semibold text-sm sm:text-base whitespace-nowrap">{l.title}</div>
                  {isActive && (
                    <div className="font-mono text-[11px] mt-1 opacity-65">{l.count} {t.capabilities.functions}</div>
                  )}
                </button>
              );
            })}
          ...
          <div className={`glass-card rounded-2xl border ${c.active} p-4 sm:p-6 lg:p-8 min-w-0`}>
            <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 mb-4 sm:mb-6">
              <div>
                <span className={`inline-block font-mono text-[11px] px-3 py-1 rounded-full border ${c.bg} ${c.border} ${c.text} mb-2 tracking-[0.16em] uppercase`}>
                  {layer.tag}
                </span>
            ...
            <p className="font-dm text-sm sm:text-base text-zs-muted leading-relaxed mb-5 sm:mb-6">{layer.desc}</p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-1.5 sm:gap-2">
              {layer.features.map((feat, i) => (
                <div key={i} className="flex items-center gap-2 p-2.5 sm:p-3 rounded-lg bg-zs-bg-3/60 border border-zs-faint/40 hover:border-zs-faint/70 transition-colors">
                  ...
                  <span className="font-dm text-[12px] sm:text-sm text-zs-text/85 leading-snug">{feat}</span>
                </div>
              ))}
            </div>
          </div>
