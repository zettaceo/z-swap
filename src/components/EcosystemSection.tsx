...
const integrations: Integration[] = [
  ...
  { name: "Z-Finance", type: "Financial Infrastructure", desc: "Compliance infrastructure, fiat custody operations, and regulated on/off-ramp connectivity.", color: "cyan" },
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid lg:grid-cols-2 gap-6 items-start">
          ...
          <div className="grid sm:grid-cols-2 gap-3">
            {integrations.slice(0, 6).map((item, i) => {
          ...
          <div className="glass-card rounded-2xl border border-zs-border overflow-hidden">
            ...
            <div className="p-4 sm:p-6 bg-zs-bg-2">
              <svg viewBox="0 0 500 400" className="w-full" fill="none" xmlns="http://www.w3.org/2000/svg">
                <defs>
                  ...
                  <radialGradient id="pg" cx="50%" cy="50%" r="50%">
                    <stop offset="0%" stopColor="rgba(0,224,135,0.14)" />
                    <stop offset="100%" stopColor="rgba(0,0,0,0)" />
                  </radialGradient>
                ...
                {[0,50,100,150,200,250,300,350,400].map((y) => (
                  <line key={`h${y}`} x1="0" y1={y} x2="500" y2={y} stroke="rgba(0,232,255,0.025)" strokeWidth="1"/>
                ))}
                {[0,50,100,150,200,250,300,350,400,450,500].map((x) => (
                  <line key={`v${x}`} x1={x} y1="0" x2={x} y2="400" stroke="rgba(0,232,255,0.025)" strokeWidth="1"/>
                ))}

                {/* Ambient glows */}
                <ellipse cx="250" cy="200" rx="120" ry="100" fill="url(#cg)" />
                <ellipse cx="430" cy="200" rx="60" ry="60" fill="url(#pg)" />
                ...
                {edges.map(([from, to], i) => {
                  ...
                  const dur = 3 + i * 0.6;
                  // Simple animated circle along line using cx/cy animation
                  const pct = [0, 0.25, 0.5, 0.75, 1];
                  return (
                    <g key={`edge-${i}`}>
                      <line
                        x1={a.x} y1={a.y} x2={b.x} y2={b.y}
                        stroke={isZ ? "rgba(0,224,135,0.22)" : "rgba(0,232,255,0.12)"}
                        strokeWidth={isZ ? "1.5" : "1"}
                        strokeDasharray={isZ ? "6 3" : "4 4"}
                      />
                      {/* Animated dot: use transform animation */}
                      <circle r="2" fill={isZ ? "#00E087" : "#00E8FF"} opacity="0.8">
                        <animate
                          attributeName="cx"
                          values={`${a.x};${b.x};${a.x}`}
                          dur={`${dur}s`}
                ...
                {Object.entries(pts).map(([id, n]) => (
                  <g key={id}>
                    ...
                    {n.highlight && (
                      <circle cx={n.x} cy={n.y} r={n.r + 8} fill="none" stroke="rgba(0,224,135,0.22)" strokeWidth="1">
                    ...
                    <circle
                      ...
                      filter={n.primary || n.highlight ? "url(#gf)" : undefined}
                    />
                    <text
                      ...
                      filter={n.primary ? "url(#gf)" : undefined}
                    >
                      {n.label}
                    </text>
                    <text
