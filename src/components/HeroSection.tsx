...
export default function HeroSection() {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      for (let i = 0; i < pts.length; i++) {
        for (let j = i + 1; j < pts.length; j++) {
          ...
          if (d < CONNECTION_DIST) {
            ctx.beginPath();
            ctx.strokeStyle = `rgba(0,232,255,${0.045 * (1 - d / CONNECTION_DIST)})`;
            ctx.lineWidth = 0.5;
      ...
      pts.forEach((p) => {
        ...
        ctx.beginPath();
        ctx.fillStyle = `rgba(0,232,255,${nearMouse ? Math.min(p.a * 2.2, 0.85) : p.a})`;
        ctx.arc(p.x, p.y, nearMouse ? p.r * 1.6 : p.r, 0, Math.PI * 2);
        ctx.fill();
      });
  ...
  return (
    <section className="relative flex flex-col min-h-screen min-h-dvh overflow-x-hidden grid-bg">
      <canvas
        ref={canvasRef}
        className="particle-canvas absolute inset-0 w-full h-full opacity-55 pointer-events-auto"
        aria-hidden="true"
      />
      ...
      <div className="absolute bottom-1/4 right-1/4 w-36 sm:w-64 lg:w-80 h-36 sm:h-64 lg:h-80 rounded-full bg-zs-cyan/[0.06] blur-[60px] lg:blur-[90px] animate-pulse-slow pointer-events-none" style={{ animationDelay: "2s" }} />
      ...
      <div className="relative z-10 flex-1 flex flex-col items-center text-center px-6 sm:px-8 lg:px-12 pt-24 sm:pt-28 md:pt-32 lg:pt-36 pb-8 sm:pb-12">
        ...
        <div className="flex flex-row items-center gap-3 sm:gap-4">
          <a href="#what-is" className="flex-shrink-0 whitespace-nowrap px-4 sm:px-7 lg:px-8 py-2.5 sm:py-3 lg:py-3.5 bg-zs-cyan text-zs-bg font-syne font-bold text-[11px] sm:text-xs lg:text-sm tracking-widest uppercase rounded-xl hover:bg-zs-cyan-dim transition-colors duration-200">
            {t.hero.cta1}
          </a>
          <a href="#architecture" className="flex-shrink-0 whitespace-nowrap px-4 sm:px-7 lg:px-8 py-2.5 sm:py-3 lg:py-3.5 border border-zs-border text-zs-text font-syne font-bold text-[11px] sm:text-xs lg:text-sm tracking-widest uppercase rounded-xl hover:border-zs-cyan/40 hover:text-zs-cyan transition-all duration-200">
            {t.hero.cta2}
          </a>
        </div>
      </div>
